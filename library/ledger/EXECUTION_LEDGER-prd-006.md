...
| AC-8 | Dashboard per-harness agent-present + plugin-enabled + Reconnect/Repair | honeycomb+hive | VERIFIED (hc API+verbs) / hive UI pending | W2 + F-2fix | endpoint now reports real `pluginEnabled` (F-2 CLOSED) + `harness status`/`repair` verbs ready + tested; hive card = hive track (may read endpoint or CLI verb) |
