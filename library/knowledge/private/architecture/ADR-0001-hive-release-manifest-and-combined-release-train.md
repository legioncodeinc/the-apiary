...
- **doctor** (`@legioncodeinc/doctor`), also published to npm with its own CI and its own OIDC release pipeline.
- **hive**, the always-on portal daemon, NOT yet published to npm.
- **nectar**, NOT yet published to npm.

Each published submodule releases on its own cadence. That independence is a feature: a honeycomb patch should not force a doctor version bump, and each product owns its own tarball, its own changelog, and its own tag namespace. But independence alone gives the fleet no notion of a *compatible set*. A user who installs "the latest of everything" can land honeycomb `A`, doctor `B`, hive `C`, and nectar `D` in a combination that no one ever tested together. As the products grow shared contracts (the loopback port map, doctor's registry schema, hive's proxy routing table, the embeddings runtime), an untested cross-product combination becomes a real support and correctness hazard.

The superproject itself has, until now, carried NO continuous integration and NO library. It was a pure aggregation point: a set of submodule pointers on a branch. Two of the four products (hive, nectar) cannot even be installed from a package registry yet, because they have no publish pipeline. The one-line installer today installs only honeycomb and doctor and then opens hive's URL without ever installing hive or nectar, which is a real coverage gap this decision must make closeable.
...
- **The manifest is the source of truth for "what ships together."** A single versioned file in the superproject (a `hive-release.json` / manifest under the superproject root, itself carrying a manifest version) pins the exact honeycomb, doctor, hive, and nectar versions that constitute a tested fleet release. The manifest is the only artifact that expresses cross-product compatibility.

- **Submodules stay independently published.** Every submodule continues to release its own npm package through its existing per-repo OIDC Trusted Publishing workflow. There is no monolithic tarball. hive and nectar each GAIN their own publish pipeline (mirroring honeycomb's and doctor's OIDC release model) so they become installable products; the manifest can only pin versions that a registry can actually serve.
...
flowchart TD
    subgraph submodules [Independently published submodules]
        ...
        hd[doctor release.yaml -> npm]
        th[hive release.yaml -> npm NEW]
        hn[nectar release.yaml -> npm NEW]
    end
...
- **A clean onboarding path for new products.** A new product joins the fleet by (a) gaining its own OIDC publish pipeline and (b) being added to the manifest. hive and nectar are the first two products to walk this path, which also closes the installer coverage gap where they were previously never installed.
...
- **Two products need new pipelines before the manifest is complete.** Until hive and nectar have publish pipelines, the manifest can only pin the two products that are published. The decision is only fully realized once all four are installable.
...
- `honeycomb/.github/workflows/release.yaml` - the existing per-submodule OIDC Trusted Publishing release the manifest pins against; the model hive and nectar copy.
...
- [`../../../../doctor/library/knowledge/private/architecture/ADR-0002-service-registration-static-registry-plus-runtime-sqlite.md`](../../../../doctor/library/knowledge/private/architecture/ADR-0002-service-registration-static-registry-plus-runtime-sqlite.md) - doctor's registry, which the installer writes when it installs a pinned set.
