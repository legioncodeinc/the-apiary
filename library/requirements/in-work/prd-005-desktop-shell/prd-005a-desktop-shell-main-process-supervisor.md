...
- **Doctor overlap — decided (ADR-0005).** Doctor keeps sole restart authority over the workloads; the shell supervises only the two roots (Doctor + Hive) and never restarts a workload. They cannot fight because ownership is single-writer: the doctor registry records one supervisor-owner per daemon and every restart is guarded by a `pidPath`-liveness no-op (see the "Reuse the existing supervisor" note above).
