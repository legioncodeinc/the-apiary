...
The manifest is a *pinning* artifact, not a build output. It names the exact `honeycomb`, `doctor`, `hive`, and `nectar` versions that constitute a tested fleet release; it never rebuilds or repackages those products.
...
- The publish pipelines that make hive and nectar pinnable (that is 001c).
...
| a-AC-2 | The manifest pins an explicit, published version string for each of the four products (`honeycomb`, `doctor`, `hive`, `nectar`); a missing or empty pin is invalid by construction. |
...
- **Shape.** A top-level `manifestVersion` plus a `products` map keyed by product slug (`honeycomb`, `doctor`, `hive`, `nectar`) to a pinned semver string. Prefer product slugs that match the installer's `--products=` tokens in PRD-002 so selection maps to a pin with no translation table.
...
- [ ] Do product keys use the bare slug (`hive`) or the npm package name (`@legioncodeinc/honeycomb`)? Slug is friendlier to `--products=`; package name is unambiguous for `npm i`.
